...
public class ContactsTest {
	...
	public void whenAddingOneContactABItIsAdded(){
		...
		mockery.checking(new Expectations() {
			{
				oneOf(box2dcontact).getWorldManifold();will(returnValue(worldManifold));
				oneOf(worldManifold).getNormal();will(returnValue(normal));
				oneOf(box2dcontact).getFixtureA();will(returnValue(fixtureA));
				oneOf(box2dcontact).getFixtureB();will(returnValue(fixtureB));
			}
		});
	...
	public void removesSecondContactIfItMatches(){
		...
		Fixture fixtureC = mockery.mock(Fixture.class);
